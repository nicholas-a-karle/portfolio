...
const CursorTracker = ({ move_time_ms = 400, move_update_ms = 10 }) => {
  ...
  const mouseMoveHandler = (event) => {
    ...
    setCursorPositions(prevPositions => {
      const newX = [...prevPositions.x, event.clientX];
      const newY = [...prevPositions.y, event.clientY];
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (cursorPositions.x.length > 0 && cursorPositions.y.length > 0) {
        ...
        if (distanceBetween <= 0.75) {
          setFollowerPosition(cursorPosition);
        } else {
  ...
  const followerSizeRaw = (isMouseClicked ? 72 : (isMousePressed ? 24 : 12)) * inverseDistance + 12 * (1 - inverseDistance) ;
  const followerSize = `${followerSizeRaw}px`;
  const cursorSize = isMouseClicked ? '8px' : (isMousePressed ? '6px' : '8px');
